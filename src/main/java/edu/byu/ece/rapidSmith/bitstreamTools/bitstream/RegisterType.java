...
public enum RegisterType {
    ...
    public static String DataToString(int data, RegisterType type) {
        if (type == CTL0) {
            return CTL0DataToString(data);
        } else if (type == COR0) {
            return COR0DataToString(data);
        } else if (type == COR1) {
            return COR1DataToString(data);
        } else if (type == CMD) {
            return CMDDataToString(data);
        } else if (type == WBSTAR) {
            return WBSTARDataToString(data);
        } else if (type == TIMER) {
            return TIMERDataToString(data);
        }
        return "";
    }
    ...
    private static String CTL0DataToString(int data) {
        ...
        if ((bitmask & data) != 0) {
            string += "Bottom ICAP Port Enabled, ";
        } else {
            string += "Top ICAP Port Enabled, ";
        }

        // Virtex 5
        bitmask = 0x00001000;            //OverTempPowerDown
        if ((bitmask & data) != 0) {
            string += "(V5 only)OverTempPowerDown enabled, ";
        } else {
            string += "(V5 only)OverTempPowerDown disabled, ";
        }

        // Virtex 5
        bitmask = 0x00000400;            //ConfigFallback
        if ((bitmask & data) != 0) {
            string += "(V5 only)ConfigFallback disabled, ";
        } else {
            string += "(V5 only)ConfigFallback enabled, ";
        }

        // Virtex 5
        bitmask = 0x00000200;            //SelectMAPAbort
        if ((bitmask & data) != 0) {
            string += "(V5 only)SelectMAPAbort disabled, ";
        } else {
            string += "(V5 only)SelectMAPAbort enabled, ";
        }
        ...
        if ((bitmask & data) != 0) {
            string += "(V5 only)DEC enabled, ";
        } else {
            string += "(V5 only)DEC disabled, ";
        }
        ...
        switch (bitmask & data) {
            case 0x00000030:
            case 0x00000020:
                string += "Rdbk Dsabld, Wr Dsabld excpt CRC, ";
                break;
            case 0x00000010:
                string += "Rdbk Dsabld, ";
                break;
        ...
        if ((bitmask & data) != 0) {
            ...
            string += "I/Os placed in high-Z state";
        }
    ...
    private static String COR0DataToString(int data) {
        ...
        if ((bitmask & data) == 0) {
            string += "(V5 only)DONE pin, ";
        } else {
            string += "Powerdown pin, ";
        }

        // Virtex 4, 5
        bitmask = 0x02000000;            //DONE_PIPE
        if ((bitmask & data) == 0) {
            string += "No pipeline stage for DONEIN, ";
        } else {
            string += "Add pipeline stage for DONEIN, ";
        }

        // Virtex 4, 5
        bitmask = 0x01000000;            //DRIVE_DONE
        if ((bitmask & data) == 0) {
            string += "DONE pin is open drain, ";
        } else {
            string += "DONE pin is actively driven high, ";
        }

        // Virtex 4, 5
        bitmask = 0x00800000;            //SINGLE
        if ((bitmask & data) == 0) {
            string += "Readback not single-shot, ";
        } else {
            string += "Readback single-shot, ";
        }
        ...
        tmp = (bitmask & data) >>> 9;
        if (tmp == 7) {
            string += "MATCH_CYCLE: No Wait, ";
        } else {
            string += "MATCH_CYCLE: " + (tmp + 1) + ", ";
        }
        ...
        tmp = (bitmask & data) >>> 6;
        if (tmp == 7) {
            string += "LOCK_CYCLE: No Wait, ";
        } else {
            string += "LOCK_CYCLE: " + (tmp + 1) + ", ";
        }
        ...
        tmp = (bitmask & data) >>> 3;
        if (tmp == 0x00000006) {
            string += "GTS tracks DONE pin";
        } else if (tmp == 0x00000007) {
            string += "GTS_CYCLE: Keep";
        } else {
            string += "GTS_CYCLE: " + (tmp + 1) + ", ";
        }
        ...
        tmp = (bitmask & data);
        if (tmp == 0x00000006) {
            string += "GWE tracks DONE pin";
        } else if (tmp == 0x00000007) {
            string += "GWE_CYCLE: Keep";
        } else {
            string += "GWE_CYCLE: " + (tmp + 1) + " ";
        }
    ...
    private static String COR1DataToString(int data) {
        ...
        if ((bitmask & data) == 0) {
            string += "PERSIST_DEASSERT_AT_DESYNCH disabled, ";
        } else {
            string += "PERSIST_DEASSERT_AT_DESYNCH enabled, ";
        }

        // Virtex 5
        bitmask = 0x00000200;            //RBCRC_NO_PIN
        if ((bitmask & data) == 0) {
            string += "RBCRC_NO_PIN disabled, ";
        } else {
            string += "RBCRC_NO_PIN enabled, ";
        }

        // Virtex 5
        bitmask = 0x00000100;            //RBCRC_EN
        if ((bitmask & data) == 0) {
            string += "RBCRC_EN disabled, ";
        } else {
            string += "RBCRC_EN enabled, ";
        }
        ...
        switch (bitmask & data) {
            case 0x00000000:
                string += "Flash mem page size: 1 byte/word";
                break;
            case 0x00000001:
                string += "Flash mem page size: 4 bytes/word";
                break;
            case 0x00000002:
                string += "Flash mem page size: 8 bytes/word";
                break;
            case 0x00000003:
                string += "Flash mem page size: Reserved";
                break;
        }
    ...
    private static String WBSTARDataToString(int data) {
        ...
        if ((bitmask & data) == 0) {
            string += "RS_TS_B disabled, ";
        } else {
            string += "RS_TS_B enabled, ";
        }
    ...
    private static String TIMERDataToString(int data) {
        ...
        if ((bitmask & data) == 0) {
            string += "TIMER_USR_MON disabled, ";
        } else {
            string += "TIMER_USR_MON enabled, ";
        }

        // Virtex 5
        bitmask = 0x01000000;            //TIMER_CFG_MON
        if ((bitmask & data) == 0) {
            string += "TIMER_CFG_MON disabled, ";
        } else {
            string += "TIMER_CFG_MON enabled, ";
        }
